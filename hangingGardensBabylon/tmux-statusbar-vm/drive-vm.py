#!/usr/bin/env python3
"""Drive the tmux-statusbar VM:

* boot it with the serial console on stdio and a QMP unix socket
* wait for the autologin root shell on the serial console
* verify (over serial) that tmux runs with a two-line status bar, that
  status-format[1] (the BOTTOM bar) contains NO session name (#S), and that
  the top bar (gruvbox status-left on status-format[0]) DOES contain it
* take QMP screendumps of the VGA console (where tmux is attached on tty1)
* exercise the session (rename a window long, add a 4th window)
...
try:
    ...
    # BOTTOM bar (status-format[1]) must contain NO session name (#S):
    # echo the count; it must be 0.
    send('echo COUNT_1=$(tmux show -gv status-format[1] | grep -o "#S" | wc -l)')
    wait_for_log(r"COUNT_1=0", 120)
    log("bottom bar contains no session name (#S) confirmed in the VM")

    # TOP bar (gruvbox status-left, used by status-format[0]) must still
    # contain the session name (#S): count must be >= 1.
    send('echo COUNT_0=$(tmux show -gqv status-left | grep -o "#S" | wc -l)')
    wait_for_log(r"COUNT_0=[1-9]", 120)
    log("top bar contains the session name (#S) confirmed in the VM")
    ...
    send("tmux send-keys -t work:2 'echo hello from the statusbar VM' Enter")
    send("tmux list-windows -t work -F '#{session_name}: #{window_name}'")
...
for name in ("vm-shot1", "vm-shot2"):
    subprocess.run(
        [MAGICK, f"/tmp/{name}.ppm", os.path.join(OUTDIR, f"{name}.png")],
        check=True,
    )
