{ frontArmToPlane : 
  { registry : (nix registry list | where flakeref == "flake:frontArmToPlane" | get ui.0 | url parse | get params | select 0 2 | transpose -r -d | update lastModified { into datetime -f "%s" } ),
    local : (nix flake metadata --json (cat /run/secrets/paths/frontArmToPlane) | from json | do 
    ...
    local_twc_input : (nix flake metadata --json (cat /run/secrets/paths/thatWaterCharmander) | from json | get locks.nodes.frontArmToPlane.locked | select rev lastModified | update lastModified { into datetime -f "%s" } ),
